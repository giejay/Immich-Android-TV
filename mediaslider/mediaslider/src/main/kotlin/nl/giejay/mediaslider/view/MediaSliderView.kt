...
class MediaSliderView(context: Context) : ConstraintLayout(context) {
        ...
        if (event.action == KeyEvent.ACTION_DOWN) {
            ...
            if (context is MediaSliderListener && (context as MediaSliderListener).onButtonPressed(event)) {
                ...
                return when (event.keyCode) {
                    KeyEvent.KEYCODE_MEDIA_PLAY -> playVideoFromRemote()
                    KeyEvent.KEYCODE_MEDIA_PAUSE -> pauseVideoFromRemote()
                    else -> togglePlayPauseFromRemote()
                }
            } else if (itemType == SliderItemType.VIDEO && isRemoteSeekForward(event.keyCode)) {
                return seekVideoBy(REMOTE_SEEK_STEP_MS)
    ...
    private fun seekVideoBy(deltaMs: Long): Boolean {
    ...
    private fun togglePlayPauseFromRemote(): Boolean {
        val player = currentPlayerInScope ?: return false
        if (player.isPlaying) {
            player.pause()
        } else {
    ...
    private fun playVideoFromRemote(): Boolean {
        val player = currentPlayerInScope ?: return false
        if (player.contentDuration > 0 && player.currentPosition >= player.contentDuration) {
            player.seekToDefaultPosition()
        }
    ...
    private fun pauseVideoFromRemote(): Boolean {
        val player = currentPlayerInScope ?: return false
        player.pause()
        return true
    }
